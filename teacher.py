...
class Teacher:
    ...
    def win(self, board, key='X'):
        ...
        if a.count('_') == 1 and a.count(key) == 2:
            ind = a.index('_')
            return ind, ind
        elif b.count('_') == 1 and b.count(key) == 2:
            ind = b.index('_')
            if ind == 0:
                return 0, 2
            elif ind == 1:
        ...
        for i in range(3):
            ...
            if c.count('_') == 1 and c.count(key) == 2:
                ind = c.index('_')
                return ind, i
            elif d.count('_') == 1 and d.count(key) == 2:
                ind = d.index('_')
                return i, ind
        return None
    ...
    def corner(self, board):
        ...
        if board[0][0] == 'O' and board[2][2] == '_':
            return 2, 2
        elif board[2][0] == 'O' and board[0][2] == '_':
            return 0, 2
        elif board[0][2] == 'O' and board[2][0] == '_':
            return 2, 0
        elif board[2][2] == 'O' and board[0][0] == '_':
            return 0, 0
