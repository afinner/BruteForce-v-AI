...
class GameLearning(object):
    ...
    def __init__(self, args, alpha=0.5, gamma=0.9, epsilon=0.1):
        ...
        if args.load:
            ...
            if args.agent_type == "q":
                agent = Qlearner(alpha,gamma,epsilon,0.,self.gridSize)
            else:
                agent = SARSAlearner(alpha,gamma,epsilon,0.,self.gridSize)
    ...
    def beginPlaying(self):
        ...
        while True:
            game = Game(self.agent,None,self.gridSize)
            game.start()
            self.games_played += 1
    ...
    def beginTeaching(self, episodes):
        ...
        teacher = TeacherN(self.ability_level, self.gridSize)
        # Train for alotted number of episodes
        while self.games_played < episodes:
            game = Game(self.agent, teacher=teacher,gridSize=self.gridSize)
            game.start()
            self.games_played += 1
            # Monitor progress
...
if __name__ == "__main__":
    ...
    parser.add_argument("-g", "--grid_size", default=3, type=int,
                        help="grid size for game ")
